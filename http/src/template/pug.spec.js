...
<html>
  ...
  // 模板文件名称
  const template_file = path.join(__dirname, "pug/index.pug");
  ...
  it("should render template file", () => {
    // 渲染模板文件
    const html = pug.renderFile(template_file, { pretty: true, ...template_args });
  ...
  it("should render template file by precompile render function", () => {
    // 预编译渲染函数
    const renderFn = pug.compileFile(template_file, { pretty: true });

    // 通过预编译函数渲染模板文件
    const html = renderFn(template_args);
