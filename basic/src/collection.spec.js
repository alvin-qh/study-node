...
describe("test 'Array'", () => {
  ...
  it("should insert elements into 'Array'", () => {
    ...
    expect(sp).to.be.empty;
    expect(arr).to.deep.eq([1, 2, 100, 200, 300, 3, 4, 5]);

    // 继续从数组下标 2 的位置删除 3 个元素, 并插入 2 个元素 (相当于替换)
    sp = arr.splice(2, 3, -1, -2);
    expect(sp).to.deep.eq([100, 200, 300]);
    expect(arr).to.deep.eq([1, 2, -1, -2, 3, 4, 5]);
  });
  ...
  it("should map and reduce elements from 'Array'", () => {
    ...
    expect(r).to.deep.eq([101, 102, 103, 104, 105]);
...
describe("test 'Iterator'", () => {
  ...
  it("should create iterable object by 'yield' function", () => {
    ...
    expect(arr).be.deep.eq([1, 2, 3, 4, 5, 6, 7, 8, 9]);

    let i = 10;
    // 对 yield 函数结果进行迭代
    for (const n of xrange(10, 20)) {
      expect(n).be.eq(i++);
    }
  });
  ...
  it("should get iterator from 'Array'", () => {
    ...
    expect(e.done).to.be.false // 获取是否完成迭代
  ...
  it("should add iterator into Class", () => {
    ...
    const arr = [...range];
    expect(arr).to.deep.eq([1, 3, 5, 7, 9]);
  });
});
...
describe("test 'Set'", () => {
  it("should use 'Set' collection", () => {
    ...
    expect(pairs).to.deep.eq([[1, 1], [3, 3], [4, 4]]);

    // 添加元素
    set.add(4);
    expect([...set]).to.deep.eq([1, 3, 4]);

    set.add(5);
    expect([...set]).to.deep.eq([1, 3, 4, 5]);
...
describe("test 'Map'", () => {
  it("should use 'Map' collection", () => {
    ...
    // 添加新的键值对
    map.set("c", 3);
    expect([...map]).to.deep.eq([["a", 1], ["b", 2], ["c", 3]]);

    // 删除键值对
    map.delete("b");
    expect([...map]).to.deep.eq([["a", 1], ["c", 3]]);

    const es = [...map.entries()];
    expect(es).to.deep.eq([["a", 1], ["c", 3]]);
