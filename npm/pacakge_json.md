...
Assuming the host complies with [semver](https://github.com/npm/node-semver#versions), only changes in the host package's major version will break your plugin. Thus, if you've worked with every 1.x version of the host package, use `"^1.0"` or `"1.x"` to express this. If you depend on features introduced in 1.5.2, use `"^1.5.2"`.
...
See [workspace](https://docs.npmjs.com/cli/v7/using-npm/workspaces) for more examples.
