 ...
 * @returns {R} 状态对象
 */
function getState<R>(key: string): R | undefined {
 ...
 * @returns 被删除的状态值
 */
function removeState(key: string): unknown {
 ...
 */
function whenStateNotExist(key: string, fn: () => void): void {
  if (!getState(key)) {
    fn();
  }
}
 ...
 */
async function whenStateNotExistAsync(key: string, fn: () => Promise<void>): Promise<void> {
  if (!getState(key)) {
    await fn();
  }
}
